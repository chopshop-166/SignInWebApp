...
def overview():
    ...
    all_users: list[User] = db.session.scalars(
        select(User).join(Role).where(Role.visible == True, Role.receives_funds == True)  # noqa: E712
    )
    total_overhead = sum(((ev.funds - ev.cost) * ev.overhead) for ev in all_events) / 100.0
